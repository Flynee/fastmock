...
router.post("/login", function(req, res) {
  ...
  loginModel
    ...
    .then((rows) => {
      ...
      console.log("sssssss", req.session)
...
function getCodeAndExpire() {
  ...
  const activeCodeMD5 = crypto
    .createHash("md5")
    .update("" + activeCodeStr)
    .digest("hex") // 密码加密
  // 计算激活码的失效时间 30分钟后过期
  ...
  try {
    ...
    if (resp3.id) {
      const sendInfo =
        ...
        "<br/>恭喜您注册成功，你在本站注册的邮箱需要验证！请点击链接激活(或者复制到浏览器打开)，完成验证！" +
        '<br/><a href="' +
        wwwBaseUrl +
        "/#/active/" +
        jsonObj.code +
...
router.get("/sendForgetPwdCode", async function(req, res) {
  ...
  const sendInfo =
    "您好，尊敬的" +
    email +
    "<br/>您正在fastmock.site找回登录密码，如果此操作并非您本人意愿，请勿透露验证码给其他人，否则可能导致域名被盗等风险！" +
    "验证码为：" +
    code
